...
esp_err_t WifiStation::connect(const char *ssid,
                               const char *password) {
...
esp_err_t WifiStation::connect() {
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        ESP_LOGI(TAG, "Failed to connect to SSID: %s", (char *)wifi_cfg.sta.ssid);
    } else {
        ESP_LOGE(TAG, "UNEXPECTED EVENT");
    }
    return ESP_OK;
...
void WifiStation::event_handler(void *arg,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        ESP_LOGI(self->TAG, "Connection to the AP fail");
